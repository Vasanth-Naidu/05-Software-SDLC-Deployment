...
* **Impact & Key Deliverables:**
  ...
  * **80%+ Cycle Time Reduction per PO:** Compressed processing time from **05–06 minutes per PO down to seconds**, drastically accelerating part fulfilment to GE Warehouses across North Americas.
...
* **Core Stack:** Visual Basic 5 (VB5 GUI & OOPs Terminal Session Objects), Hummingbird Host-Explorer (EHLLAPI/ OLE Automation Drivers), Mainframe Terminal Sessions (3270 Emulation), FTP File Parser (Automated Report Ingestion), MS Excel API.
...
The Global Asset Management (GAM) India team within GEMS USA managed proactive inventory replenishment across GE Warehouses across North Americas. Frontline representatives manually converted Material Requests (MR) into official Purchase Orders (POs) inside the legacy mainframe:
...
│  │   (Raw MR Data Reports)  │                      │ & Line-Item Parser (VB5)       │  │
│  └────────────┬─────────────┘                      └──────────────┬─────────────────┘  │
└───────────────┼───────────────────────────────────────────────────┼────────────────────┘
                │                                                   │
                ▼                                                   ▼
...
└───────────────────────────────────────┬────────────────────────────────────────────────┘
                                        ...
                                        ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
│                   AUTOMATED MS EXCEL PO TRACKING LOG & EXCEPTION REPORT                │
├────────────────────────────────────────────────────────────────────────────────────────┤
...
1. **Automated FTP Ingestion & Parsing:** Programmatically connected to GEMS FTP servers, pulled daily MR reports, and loaded pending parts directly into the VB5 memory buffer.
...
5. **Auto-Updating Audit Log:** Automatically updated Excel fulfilment logs upon order completion, removing the need for manual post-entry spreadsheet maintenance.

---

## 3. Operational Modules:

* **Module 1: FTP Ingester & File Parser:** Automatically retrieves daily MR data files from GEMS FTP servers and parses raw text lines into structured memory objects.
...
* **Module 4: Terminal Injection & Excel Sync Module:** Fires automated keystrokes into the Hummingbird 3270 terminal instance and writes completed PO numbers back to Excel tracking logs.
...
| **Tracking Maintenance** | Reps manually updated Excel after each PO | **Automated Excel Log Synchronisation** | Completely removed post-entry spreadsheet logging. |
...
* **Unified UI/UX Ergonomics:** Designing single-screen workspaces that merge disparate data sources (FTP, Excel, Mainframes) into intuitive decision hubs for operational users.
