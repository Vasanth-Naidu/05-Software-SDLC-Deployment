...
* **The Strategic Objective:** Digitally transform Cendant Mobility’s manual invoice audit lifecycle by converting 1,000+ pages of complex corporate relocation contract pricing books and client policy exhibits into an automated validation engine. The target was to eliminate subjective audit errors, drastically accelerate line-item audit throughput, and clear massive historical audit backlogs.
* **The Big Problem:** Relocation expense audits required cross-referencing multi-layer line items against client-specific Relocation Management Agreements (RMAs). Manual binder lookups caused extreme operational friction—baseline throughput sat at just 2 to 4 audits/day against strict team target of 8 audits/day, putting high-accuracy auditors at risk of performance management.
* **The Solution:**
  * **Phase 1 (Self-Help Proof of Concept):** Decomposed complex contract policies into 6 core invoice inputs using MS Excel and VBA, boosting personal output from 2 to 10 audits/day (a 5x jump).
...
* **Core Value Delivered:**
  ...
  * **Six Sigma Green Belt Certification:** Formally chartered and executed as a GE Six Sigma Green Belt project, capturing client executive attention and prompting an on-site Cendant delegation visit to GECIS India to experience GE's continuous improvement culture first-hand.
* **Tools & Stack:** Visual Basic 5 (VB5 Front-End GUI), SQL Database (Contract Rules & Audit Logs), MS Excel / VBA (Initial Prototype Engine), Process Decomposition (6-Point Validation Model).
...
The **Cendant Wizard** transformed manual binder lookup into a deterministic 6-point data input engine:

```text
┌──────────────────────────────────────────────┐     ┌──────────────────────────────────────────────┐
│  1. 6-Point Invoice Data Input               │     │  2. VB5 / SQL Contract Rules Engine          │
│ Ingest 6 key invoice parameters (Amount,     │ ──> │ Evaluate inputs against digitised SQL tables │
│ Vendor Code, State, Move Type, Tier, Date).  │     │ containing 1,000+ pages of contract terms.   │
└──────────────────────────────────────────────┘     └──────────────────────┬───────────────────────┘
                                   ┌────────────────────────────────────────┘
                                   ▼
┌──────────────────────────────────────────────┐     ┌──────────────────────────────────────────────┐
│  3. Automated Line-Item Exception Engine     │     │  4. Client Audit Finding & Recovery Log      │
│ Flag unapproved charges, overcharges, or     │ ──> │ Instantly generate standardised audit reports│
│ missing contract discounts automatically.    │     │ for client recovery and line-item dispute.   │
└──────────────────────────────────────────────┘     └──────────────────────────────────────────────┘
...
* **Module 1: 6-Point Rapid Input GUI (VB5):** Desktop interface designed to minimise keystrokes by accepting only the 6 core invoice parameters required for line-item computation.
* **Module 2: SQL Contract Rules Matrix:** Relational database storing digitised pricing logic, state-by-state tax variations, and client-specific fee caps across 1,000+ contract pages.
* **Module 3: Exception & Variance Detector:** Automatically compares invoice totals against computed contractual allowances, highlighting discrepancies as immediate audit findings.
* **Module 4: Audit Reporting & Quality Dashboard:** Exports standardised audit summaries and line-item dispute logs ready for client submission.
...
| --- | --- | --- | --- |
| **Daily Audit Throughput** | 02–04 audits/ auditor/ day | **10+ audits / auditor / day** | **5x productivity jump** across the operational line. |
| **Audit Methodology** | Manual lookup across 1,000+ binder pages | **6-point automated input parsing** | Reduced validation time from hours to minutes per contract. |
...
* **Process Decomposition & Digitisation:** Mapped 1,000+ pages of unstructured contract rules into an elegant 6-point data entry logic model.
